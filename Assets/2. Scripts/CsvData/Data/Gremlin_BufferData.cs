...
[Serializable]
public class Gremlin_BufferData : ICSVLoad, ITableKey
{
    public Rarity Gremlin_Tier { get; set; }
    public float Gremlin_Level_Bonus { get; set; }
    public float Gremlin_Tier_Cooltime { get; set; }
    ...
    public void LoadFromCsv(string[] values)
    {
        ...
        // 2: Gramlin_Tier_Cooltime (float)
        if (values.Length > 2 && float.TryParse(values[2], out float v2)) Gremlin_Tier_Cooltime = v2;
    }
}
